    ...
    function crypt(message, key, operation) {
        ...
        return vectorDigraphs.map(function (value, index, array) {
            ...
            if (firstChar.col === secondChar.col) {
                return (grade[(firstChar.row + operation) % 5][firstChar.col] + grade[(secondChar.row + operation) % 5][secondChar.col]);
            } else if (firstChar.row === secondChar.row) {
                return (grade[firstChar.row][(firstChar.col + operation) % 5] + grade[secondChar.row][(secondChar.col + operation) % 5]);
            } else {
